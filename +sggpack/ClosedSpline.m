...
%
% Class representing a closed cubic spline curve in R2. Provides parametric
% mapping [x(t),y(t)] on 0<t<1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
classdef ClosedSpline < handle
    ...
    methods (Access = public)
        ...
        function r  = interpolate(obj,t)
            ...
            for k=1:n
                ...
                dist = t(k)*obj.N; %dist @ i=N is i*N
                if (dist >= obj.N)
                    i=1;
                else
                    i = floor(dist)+1;
...
function [a,b,c,d] = spline_coefficients(f)
    ...
    while res>1.0E-06
        ...
        rhs(1) = 3*(f(2)-f(N)) - B(N); 
        rhs(N) = 3*(f(1)-f(N-1)) - B(1);
        
        
        B = beta*thomas3(a,b,c,rhs)  + (1-beta)*B;
        
        res = max(abs(B(N)   + 4*B(1) + B(2) - 3*(f(2)-f(N))), ...
                  abs(B(N-1) + 4*B(N) + B(1) - 3*(f(1)-f(N-1))));
%         fprintf('res = %f\n',res);      
    end
    b = B;
    a = f;
    ...
    c(N) = 3*(f(1)-f(N)) - 2*b(N) - b(1);
    d(1:N-1) = 2*(f(1:N-1)-f(2:N)) + b(1:N-1) + b(2:N);
    d(N) = 2*(f(N)-f(1)) + b(N) + b(1);
end
